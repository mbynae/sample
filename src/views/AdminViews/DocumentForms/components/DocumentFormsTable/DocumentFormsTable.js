...
const DocumentFormsTable = props => {
	const classes = useStyles();
	const { className, history, menuKey, rootUrl, documentForms, getDocumentForms, categories, pageInfo, setPageInfo, staticContext, ...rest } = props;
	...
	const handlePageChange = (event, page) => {
		...
		getDocumentForms(page, pageInfo.size);
	};
	...
	const handleRowsPerPageChange = event => {
		...
		getDocumentForms(0, event.target.value);
	};
											...
											documentForms.length === 0 ?
												<MC.TableRow hover>
													<MC.TableCell colSpan={2} align="center">
														조회된 문서양식 데이터가 한 건도 없네요.
													</MC.TableCell>
												</MC.TableRow>
												:
												documentForms.slice(slice).map(objView)
										)
										:
										<MC.TableRow hover>
											<MC.TableCell colSpan={2} align="center">
