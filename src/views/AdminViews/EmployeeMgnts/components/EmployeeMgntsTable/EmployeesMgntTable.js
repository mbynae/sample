...
const EmployeeMgntsTable = props => {
	const classes = useStyles();
	const { className, history, menuKey, aptId, rootUrl, employeeMgnts, getEmployeeMgnts, departments, getDepartments, pageInfo, setPageInfo, staticContext, ...rest } = props;
	...
	const handleSelectAll = event => {
		let selectedList;
		event.target.checked ? selectedList = employeeMgnts.map(employeeMgnt => employeeMgnt) : selectedList = [];
	...
	const handlePageChange = (event, page) => {
		...
		getEmployeeMgnts(page, pageInfo.size);
	};
	...
	const handleRowsPerPageChange = event => {
		...
		getEmployeeMgnts(0, event.target.value);
	};

	const handleRowClick = (obj) => {
		history.push(`${rootUrl}/employeeMgnt/${obj.id}`);
	};

	const removeObject = async (id) => {
		return employeeMgntRepository.removeEmployeeMgnt(id, { aptId: aptId });
	};
		...
		handleAlertToggle(
			...
			async () => {
				...
				handleAlertToggle(
					"isOpen",
					"삭제완료",
					"선택된 직원 정보 모두를 삭제 하였습니다.",
					() => {
						setSelectedObjects([]);
						setAlertOpens({ ...alertOpens, isOpen: false });
					}
				);
				await getEmployeeMgnts(pageInfo.page, pageInfo.size);
			},
			() => {
				// 삭제안하기
	...
	const handleOpenRegisterPage = event => {
		if ( departments.length === 0 ) {
			handleAlertToggle(
				"isOpen",
				"부서 및 직책 등록 필요",
				"부서 및 직책을 먼저 등록이 필요합니다. 부서 및 직책을 먼저 등록 해주세요.",
				() => {
					setAlertOpens({ ...alertOpens, isOpen: false });
				}
			);
		} else {
			history.push(`${rootUrl}/employeeMgnt/edit`);
		}
	};
								...
								<MC.TableRow>
									<MC.TableCell align={"center"} padding={"checkbox"}>
										<MC.Checkbox
											checked={
												employeeMgnts ?
													(employeeMgnts.length === 0 ? false : selectedObjects.length === employeeMgnts.length) : false
											}
											color={"primary"}
											indeterminate={
												selectedObjects.length > 0
												&& selectedObjects.length < (employeeMgnts ? employeeMgnts.length : 10)
											}
											onChange={handleSelectAll}
										/>
									</MC.TableCell>
									<MC.TableCell align={"center"}>부서</MC.TableCell>
									<MC.TableCell align={"center"}>직책</MC.TableCell>
									<MC.TableCell align={"center"}>이름</MC.TableCell>
									<MC.TableCell align={"center"}>휴대폰번호</MC.TableCell>
									<MC.TableCell align={"center"}>내선번호</MC.TableCell>
							...
							<MC.TableBody>
								{
									employeeMgnts ?
										(
											employeeMgnts.length === 0 ?
												<MC.TableRow hover>
													<MC.TableCell colSpan={7} align="center">
														조회된 직원 데이터가 한 건도 없네요.
													</MC.TableCell>
												</MC.TableRow>
												:
												employeeMgnts.slice(slice).map(objView)
										)
										:
										<MC.TableRow hover>
											<MC.TableCell colSpan={7} align="center">
...
export default EmployeeMgntsTable;
